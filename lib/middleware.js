...
// creates middleware for authorizing a zcap policy request
export function authorizeZcapPolicyRequest({
  expectedAction, root = false
} = {}) {
  ...
  return authorizeZcapInvocation({
    async getExpectedValues({req}) {
      const {profileId, policyId} = req.params;
      const policyRoot =
        `${profilePath}/${encodeURIComponent(profileId)}/zcaps/policies`;
      return {
        // allow expected action override
        action: expectedAction,
        host: bedrock.config.server.host,
        rootInvocationTarget: root ?
          policyRoot : `${policyRoot}/${encodeURIComponent(policyId)}`
      };
    },
