...
export function verifyRefreshableZcapDelegation() {
  return asyncHandler(async function verifyRefreshZcap(req, res, next) {
    ...
    // `profileId` and `delegateId` params always present where this middleware
    // is used
    const {profileId, delegateId} = req.params;

    // confirm capability to be refreshed has `controller` matching the
    // `delegateId` of the route
    if(capability.controller !== delegateId) {
      throw new BedrockError(
        `The controller "${capability.controller}" of the capability to be ` +
        `refreshed must match the HTTP route delegate ID "${delegateId}".`, {
    ...
    try {
      const results = await _verifyDelegation({
        ...
        suiteFactory,
        // allow expired zcap to be refreshed
        allowExpired: true
      });
...
async function _verifyDelegation({
  ...
  const date = allowExpired ? new Date(capability.expires) : undefined;
