...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req/*, res*/) => {
      ...
      // FIXME: first check cache for already refreshed zcap
      /*
      const {capability} = req.body;
      const cacheKey = getRefreshableZcapCacheKey({
        profileId, delegatorId, capability
      });
      // FIXME: details:
      /*const cacheKey = JSON.stringify({
        profileId, delegatorId, hash: sha256(jcs(capability))
      });*/

      // FIXME: move code below into _getUncachedRefreshedZcap()
      ...
      /*
      // FIXME: get the profile agent from the policy
      ...
      const {capability} = req.body;
      const delegated = await profileAgents.refreshCapability({
