...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const profile = await profiles.create({
        accountId: account, didMethod, didOptions
      });
      res.json(profile);
    }));
  ...
  app.post(
    ...
    ensureAuthenticated,
    // validate('profile.createProfileAgent'),
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      ...
      const profileAgentDetails = {profileId: profile};
      if(account) {
  ...
  app.get(
    routes.profileAgents,
    ensureAuthenticated,
    // validate({query:'profile.getProfileAgents'}),
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
  ...
  app.get(
    routes.profileAgent,
    ensureAuthenticated,
    // validate({query: 'profile.byAccount'}),
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
  ...
  app.post(
    ...
    ensureAuthenticated,
    // validate('profile.delegateCapability'),
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      ...
      if(!invoker) {
        throw new BedrockError(
          'The "invoker" property in the body must be specified.',
          'DataError',
          {invoker, httpStatusCode: 400, public: true});
      }
      // TODO: Add permissions to allow access for admin
      if(account !== accountId) {
        throw new BedrockError(
  ...
  app.post(
    ...
    ensureAuthenticated,
    // validate({query: 'profile.byAccount', body: 'profile.zcaps'}),
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      const {account} = req.query;
