...
let privateKmsBaseUrl;
let publicKmsBaseUrl;

bedrock.events.on('bedrock.init', () => {
  if(cfg.kms.private.port === 443) {
    privateKmsBaseUrl = `https://${cfg.kms.private.host}/kms`;
  } else {
    privateKmsBaseUrl =
      `https://${cfg.kms.private.host}:${cfg.kms.private.port}/kms`;
  }
  if(cfg.kms.public.port === 443) {
    publicKmsBaseUrl = `https://${cfg.kms.public.host}/kms`;
  } else {
    publicKmsBaseUrl =
      `https://${cfg.kms.public.host}:${cfg.kms.public.port}/kms`;
  }
});
