...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {account, didMethod, didMethodMode} = req.body;
      if(!account) {
      ...
      const profile = await profiles.create({
        accountId: account, didMethod, didMethodMode
      });
      res.json(profile);
    }));
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      const {account, profile, token, didMethod, didMethodMode} = req.body;
      ...
      const profileAgentDetails = {
        profileId: profile, didMethod, didMethodMode
      };
      if(account) {
