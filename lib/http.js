...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  // FIXME: this needs to be a POST.
  app.get(
    routes.profileAgentCapabilities,
    ensureAuthenticated,
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      // FIXME: these can be in the body in the POST implementation
      // FIXME: rename ID to `invoker` because it's the invoker of the
      // delegation.
  ...
  // FIXME: is this API obsolete since capabilities are included in the
  // profileAgent itself and are available via GET profileAgent?
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {profileAgentId} = req.params;
      const {profileAgent: {zcaps}} = await profileAgents.get(
        {id: profileAgentId});
      res.json(zcaps);
    }));

  // update profile agent's zcaps (updates their capability set)
  // FIXME: will this API also handle the removal of zcaps?
  app.post(
    routes.profileAgentCapabilitySet,
    ensureAuthenticated,
    // TODO: add validation
    asyncHandler(async (req, res) => {
      ...
      // augment or overwrite existing zcaps
      Object.assign(profileAgent.zcaps, zcaps);
