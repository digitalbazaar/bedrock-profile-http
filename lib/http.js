...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const profile = await profiles.create({actor: null, accountId, settings});
      res.json(profile);
    }));
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      const {account: {id: accountId}} = (req.user || {});
      const profileRecords = await profiles.getAll({actor: null, accountId});
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const profile = await profiles.get({actor: null, accountId, profileId});
      res.json(profile);
    }));
